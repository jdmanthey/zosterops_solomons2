options(scipen=999)
project_directory <- "/lustre/scratch/jmanthey/02_zosterops/06_phylogenies"
directory_name <- "15_mrbayes"
cluster <- "nocona"
max_number_jobs <- 800
...
vcf_header <- paste0(project_directory, "/OZ246513.1.recode.vcf.gz")

# name of script to write
output_script_name <- "01_mrbayes_array.sh"
...
write(paste("#SBATCH --job-name=", "mrbayes", sep=""), file=a.script, append=T)
...
write("#SBATCH --mem-per-cpu=4G", file=a.script, append=T)
...
write("", file=a.script, append=T)
write("\tsource activate bcftools", file=a.script, append=T)
write("", file=a.script, append=T)
...
rscript_command <- paste("\tRscript _write_mrbayes.r ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
...
write("", file=a.script, append=T)
write("\tconda deactivate", file=a.script, append=T)
write("", file=a.script, append=T)

# mrbayes command
mrbayes_command <- paste0("\tmb ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex")
write(mrbayes_command, file=a.script, append=T)
write("", file=a.script, append=T)
# remove unnecessary files at end
write(paste("\trm ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex", sep=""), file=a.script, append=T)
write(paste("\trm ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex.ckp", sep=""), file=a.script, append=T)
write(paste("\trm ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex.ckp~", sep=""), file=a.script, append=T)
write(paste("\trm ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex.mcmc", sep=""), file=a.script, append=T)
write(paste("\trm ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex.parts", sep=""), file=a.script, append=T)
write(paste("\trm ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex.trprobs", sep=""), file=a.script, append=T)
write(paste("\trm ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex.tstat", sep=""), file=a.script, append=T)
write(paste("\trm ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.nex.vstat", sep=""), file=a.script, append=T)
